...
Provide a rephrased version that is easier to understand while maintaining accuracy.`;
  ...
  const aiProvider = (localStorage.getItem('aiProvider') as 'ollama' | 'groq') || 'ollama';
  const ollamaModel = localStorage.getItem('ollamaModel') || 'mistral:latest';
  
  return await generateAIResponse(prompt, mockResults, 'search', aiProvider, ollamaModel);
}
